	...
	public function fetch_categories()
	{
		...
		if($q->num_rows() > 0) {
			...
			foreach ($q->result() as $key => $value) {
				...
				$q2 = $this->db->select('id, name, image_path AS image, has_subcategories AS isSubCategoryEnabled, multiplier_enabled, multiplier_value')->where('parent_category_id', $parent_category_id)->get('categories');
				if($q2->num_rows() > 0) {
	...
	public function compare_requirements()
	{
		...
		if($q->num_rows() > 0) {
			...
			foreach ($explode_requirement_ids as $key => $value) {
				...
				// }
				array_push($sub_total, $total != NUll && !empty($total) ? number_format($total, 2)."" : 0.00);
