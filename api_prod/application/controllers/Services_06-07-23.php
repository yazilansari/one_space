...
class Services extends CI_Controller {
	...
	public function store_requirement()
	{
		...
		// This method has several options, check the source code documentation for more information.
		$requirement_quotation_path = $_SERVER['DOCUMENT_ROOT'].'requirement_quotation/project'.$request->project_id.'_requirement'.$max_req_no.'.pdf';
	...
	public function sign_PDF() {
		...
		// if($pdf->Output($signed_pdf_path, 'F')) {
		$response['success'] = true;
		...
		// } else {
		// 	$response['success'] = false;
		// 	$response['message'] = 'Error Occurred While Signing PDF.';
		// 	header('Content-Type: application/json; charset=utf-8');
		// 	echo json_encode($response);exit();
		// };
	}
